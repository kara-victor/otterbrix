...
namespace {
    ...
    using components::collection_full_name_t;
    ...
    compare_expression_ptr eq_join_id(std::pmr::memory_resource* resource) {
        return make_compare_expression(resource,
                                       compare_type::eq,
                                       components::expressions::key_t(resource, "id", side_t::left),
                                       components::expressions::key_t(resource, "id_col1", side_t::right));
    }

    compare_expression_ptr left_score_gte(std::pmr::memory_resource* resource, uint16_t param) {
        return make_compare_expression(
            resource, compare_type::gte, components::expressions::key_t(resource, "score", side_t::left), core::parameter_id_t(param));
    }

    compare_expression_ptr right_score_gte(std::pmr::memory_resource* resource, uint16_t param) {
        return make_compare_expression(
            resource, compare_type::gte, components::expressions::key_t(resource, "score", side_t::right), core::parameter_id_t(param));
    }

    compare_expression_ptr unknown_constant(std::pmr::memory_resource* resource, uint16_t param) {
        return make_compare_expression(
            resource, compare_type::gte, components::expressions::key_t(resource, "limit", side_t::undefined), core::parameter_id_t(param));
    }
...
TEST_CASE("components::optimizer::push_down: predicates using both sides stay on join") {
    ...
    plan->append_expression(make_compare_expression(&pool,
                                                    compare_type::gte,
                                                    components::expressions::key_t(&pool, "left_score", side_t::left),
                                                    components::expressions::key_t(&pool, "right_score", side_t::right)));
