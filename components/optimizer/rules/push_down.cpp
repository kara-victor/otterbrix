...
namespace components::optimizer::rules {
    ...
    namespace {
        ...
        bool has_left_reference(const compare_expression_ptr& expression) {
            return expression->primary_key().side() == side_t::left || expression->secondary_key().side() == side_t::left;
        }

        bool has_right_reference(const compare_expression_ptr& expression) {
            return expression->primary_key().side() == side_t::right || expression->secondary_key().side() == side_t::right;
        }
    ...
    std::pmr::vector<expression_ptr>
    push_down_t::split_and_conjuncts(std::pmr::memory_resource* resource, const expression_ptr& expression) {
